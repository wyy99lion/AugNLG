...
def preprocess(string, replace_num=True, tokenization=True):
    ...
    if replace_num:
        ...
        for num_len in sorted(number_length_dict.items(), key = lambda d:d[0], reverse = True):
            # .items()以列表返回可遍历的(键, 值) 元组数组。然后按数字长度排序
            for number in num_len[1]:#返回每个值
...
def ngram_validation_check(ngram):
    # ngram 验证检查。如果ngram关键词任意一个为空或长度为1，返回false
    for tok in ngram:
        if len(tok) <= 1:
            return False
    return True
...
def get_ngrams(string, n):
    ...
    for i in range(0, len(flist)-n+1):
        if not ngram_validation_check(flist[i:i+n]):
            continue
            #如果i对应的ngram为空或1，即其中词长度<=1，跳出循环
        ngram = ' '.join(flist[i:i+n]) #将i至i+n个词定义为i对应的ngram
        ngrams.append(ngram) #将符合条件的ngram添加到ngrams,最后一个ngram长度可能为2
    return ngrams
...
if __name__ == '__main__':
    ...
    for line in open(in_domain_path):
        ...
        #样例 inform_no_match ( pricerange = pricey ; acceptscreditcards = no ) 
            #  & there are no hotel -s in pricey price range that do not accept card -s
        #strip()方法用于移除字符串头尾指定的字符(默认为空格)。
        #.split(' & ') 使用分隔符' & '分割,前面为mr,后面为句子
        if len(flist) != 2:
            continue
            #如果其line中字符串数量（句子数量）不为2，跳出循环
        utterance = flist[1].strip() #选取该行第二个句子，即生成的句子，去除首位字符
        in_domain_exps.append(utterance) #将所有句子添加到in_domain_exps
